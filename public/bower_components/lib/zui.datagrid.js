...
(function ($, undefined) {
    ...
    DataGrid.prototype.renderCell = function (rowIndex, colIndex, $row) {
        ...
        if (options.onCellClick) {
            var cellData = cell;
            options.onCellClick(cellData, $cell, that);
        }

        return $cell;
    };

    DataGrid.prototype.getDataIndex = function (id) {
        var that = this;
        var items = this.dataSource.array;
        var index;
    ...
    DataGrid.prototype.getDataIndexAndType = function (id,orderTypeId) {
        var that = this;
        var items = this.dataSource.array;
        var index;
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            if (item.id == id && item.orderTypeId  == orderTypeId) {
                index = i;
            }
        }
        return index;
    }
    DataGrid.prototype.getDataByType = function (type) {
        var that = this;
        var items = this.dataSource.array;
        var index;
        for (var i = 0; i < items.length; i++) {
    ...
    DataGrid.prototype.getDataIndexByKey = function (key, id) {
        var that = this;
        var items = this.dataSource.array;
        var index;
        for (var i = 0; i < items.length; i++) {
